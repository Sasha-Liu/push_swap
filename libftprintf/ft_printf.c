...
/*   Updated: 2022/12/27 20:00:55 by hsliu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_print(va_list ap, const char *fmt, int *cnt, int *err)
{
	...
	{
		if (*fmt != '%')
			*cnt = *cnt + ft_putchar_err(*fmt, err);
		else
		{
			fmt++;
			if (*fmt == 'c' || *fmt == 'd' || *fmt == 'i')
				*cnt = *cnt + ft_print_int(va_arg(ap, int), *fmt, err);
			else if (*fmt == 's' || *fmt == 'p')
				*cnt = *cnt + ft_print_ptr(va_arg(ap, void *), *fmt, err);
